...
public class PartnerStoreImpl implements PartnerStore {
    ...
    public Partner store(Partner partner) {
        if (StringUtils.isEmpty(partner.getToken())) throw new InvalidParamException("partner.getPartnerToken()");
        if (StringUtils.isEmpty(partner.getName())) throw new InvalidParamException("partner.getPartnerName()");
